...
class Trainer(BaseConfigs):
    ...
    # GPT-1 configurations (matching GPT-1 architecture)
    layer_norm_epsilon: float = 1e-05
    d_model: int = 768  # same as GPT-1
    n_layers: int = 12  # GPT-1 has 12 layers
    n_heads: int = 12  # same number of attention heads as GPT-1
    n_positions: int = 512  # GPT-1 uses shorter context length
    vocab_size: int = 50257  # Same vocabulary size as GPT-2

    # Training configs
    epochs: int = 10
    batch_size: int = 16  #modificar el batch size, original 32
